      ...
      {parts.map((part, i) => {
        if (part.startsWith("```")) {
          ...
          return (
            <pre
              key={i}
              className="bg-[#0a0a0a] rounded-xl p-4 overflow-x-auto text-xs font-mono text-emerald-300 border border-white/5 mt-2"
            >
              {lang && (
                <div className="text-zinc-500 text-[10px] uppercase tracking-widest mb-2 font-sans">
                  {lang}
                </div>
              )}
              <code>{code}</code>
            ...
            {inlineParts.map((s, j) => {
              if (s.startsWith("`") && s.endsWith("`"))
                return (
                  <code
                    key={j}
                    className="bg-white/10 text-emerald-300 px-1.5 py-0.5 rounded font-mono text-xs"
                  >
                    {s.slice(1, -1)}
                  </code>
                );
  ...
  return (
    <div className="space-y-2">
      {[...Array(5)].map((_, i) => (
        <div
          key={i}
          className="p-3 rounded-xl bg-white/[0.03] border border-white/5 animate-pulse"
        >
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      setGithubProfile({
        username: profile.login ?? fallbackProfile.username,
        name: profile.name ?? fallbackProfile.name,
        avatarUrl: profile.avatar_url ?? fallbackProfile.avatarUrl,
        profileUrl: profile.html_url ?? fallbackProfile.profileUrl,
    ...
    try {
      ...
      if (treeRes.ok) {
        ...
        const allFiles: string[] =
          (treeData.tree as GitHubTreeItem[] | undefined)
        ...
        const importantFiles = allFiles
          .filter(
            (f) =>
              /\.(ts|tsx|js|jsx|py|go|rs|java|md|json|toml|yaml|yml)$/.test(
                f,
              ) &&
      ...
      setMessages([
        {
          role: "assistant",
          content: `I've analysed **${repo.name}** — loaded the file tree and key source files. What would you like to know?`,
        },
      ]);
    } catch {}
    setProcessing(false);
      ...
      <header className="h-14 flex-shrink-0 flex items-center justify-between px-5 border-b border-white/[0.07] bg-[#0a0a0a]/80 backdrop-blur-xl z-50 relative">
        <div className="flex items-center gap-2.5">
          ...
          <div
            ...
            style={{
              backgroundImage:
                "url(https://ik.imagekit.io/sl226drpx/grok-image-da915e64-fb0b-4722-9896-e90a54cc9528-removebg-preview.png)",
            }}
          />
          ...
          <span
            className="text-[10px] px-1.5 py-0.5 rounded-md bg-emerald-500/15 text-emerald-400 border border-emerald-500/20 font-medium tracking-wide"
        ...
        <div className="flex items-center gap-3 relative">
          ...
          >
            {githubProfile?.avatarUrl ? (
              <div
                ...
                className="w-7 h-7 rounded-lg bg-cover bg-center ring-1 ring-white/10"
                style={{ backgroundImage: `url(${githubProfile.avatarUrl})` }}
              />
            ) : (
              <div
                ...
                style={{
                  ...
                  boxShadow: "0 2px 8px -2px rgba(16,185,129,0.45)",
                }}
              >
                {userInitials}
              </div>
          ...
          {userMenuOpen && (
            <div
              className="absolute top-full right-0 mt-2 w-64 bg-[#141414] border border-white/[0.08] rounded-2xl shadow-2xl shadow-black/60 p-2 z-50"
              onClick={() => setUserMenuOpen(false)}
            >
              <div className="flex items-center gap-3 rounded-xl bg-white/[0.03] border border-white/[0.06] p-3 mb-2">
                {githubProfile?.avatarUrl ? (
                  <div
                    className="w-11 h-11 rounded-xl bg-cover bg-center ring-1 ring-white/10 flex-shrink-0"
                    style={{
                      backgroundImage: `url(${githubProfile.avatarUrl})`,
                    }}
                  />
                ) : (
                  <div className="w-11 h-11 rounded-xl bg-emerald-500/15 border border-emerald-500/20 flex items-center justify-center text-sm font-bold text-emerald-300 flex-shrink-0">
                    {userInitials}
                  </div>
                )}
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="p-3 border-b border-white/[0.06] space-y-3">
            <div className="rounded-2xl border border-white/[0.08] bg-white/[0.035] p-3 shadow-2xl shadow-black/20">
              <div className="flex items-center gap-3">
                {githubProfile?.avatarUrl ? (
                  <div
                    className="w-12 h-12 rounded-2xl bg-cover bg-center ring-1 ring-white/10 flex-shrink-0"
                    style={{
                      backgroundImage: `url(${githubProfile.avatarUrl})`,
                    }}
                  />
                ) : (
                  <div className="w-12 h-12 rounded-2xl bg-emerald-500/15 border border-emerald-500/20 flex items-center justify-center text-sm font-bold text-emerald-300 flex-shrink-0">
                    {userInitials}
                  </div>
                )}
                <div className="min-w-0">
                  ...
                  <a
                    href={githubProfile?.profileUrl ?? "https://github.com"}
                    target="_blank"
                    rel="noopener noreferrer"
              ...
              <div className="grid grid-cols-2 gap-2 mt-3">
                <div className="rounded-xl bg-black/20 border border-white/[0.06] px-2.5 py-2">
                  <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wide text-zinc-500">
                    <BookOpen size={11} /> Repos
                  </div>
                  <p className="text-sm font-semibold text-white mt-0.5">
                    {githubProfile?.publicRepos ?? repos.length}
                  </p>
                </div>
                <div className="rounded-xl bg-black/20 border border-white/[0.06] px-2.5 py-2">
                  <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wide text-zinc-500">
            ...
            {fetchingRepos ? (
                    ...
                    style={
                      isActive
                        ? {
                            ...
                            boxShadow: "0 1px 12px -4px rgba(16,185,129,0.2)",
                          }
                        : {}
                    }
                  >
                    ...
                    {repo.description && (
                      <p className="text-[11px] text-zinc-500 line-clamp-2 leading-relaxed mb-1.5">
                        {repo.description}
                      </p>
                    )}
              ...
              <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
                ...
                <p className="text-[11px] text-zinc-600 leading-relaxed">
                  Connect your GitHub account and load your repositories to get
                  started
                </p>
              </div>
            )}
        ...
        <main className="flex-1 flex flex-col min-w-0 relative">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
            {messages.length === 0 && !selectedRepo && (
              <div className="flex flex-col items-center justify-center h-full gap-6 text-center max-w-md mx-auto">
                ...
                <div className="relative">
                  ...
                  />
                  <div
                    className="relative w-20 h-20 rounded-3xl border flex items-center justify-center"
                    style={{
                      background: "rgba(16,185,129,0.08)",
                ...
                <div>
                  ...
                  <p className="text-zinc-500 text-sm leading-relaxed">
                    Select a repository from the sidebar, then ask questions
                    about the codebase, architecture, or specific files.
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-2 w-full">
                  {[
                    ...
                    <div
                      key={s}
                      className="px-3 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.07] text-xs text-zinc-500 text-left hover:border-emerald-500/25 hover:bg-emerald-500/[0.04] hover:text-zinc-300 cursor-default transition-all"
                    >
                      {s}
                    </div>
                  ))}
            ...
            {messages.map((msg, i) => (
              ...
              >
                    ...
                    msg.role === "user"
                      ? {
                          background:
                            "linear-gradient(135deg, #10b981 0%, #059669 100%)",
                          boxShadow:
                            "0 0 0 1px rgba(16,185,129,0.25), 0 8px 32px -8px rgba(16,185,129,0.35)",
                        }
                      : {}
                  }
                >
                ...
                {msg.role === "user" && (
                  <div
                    className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 text-[10px] font-bold text-white"
                    style={{
                      background:
                        "linear-gradient(135deg, #10b981 0%, #059669 100%)",
          ...
          <div className="flex-shrink-0 px-6 pb-6 pt-3">
            ...
            <div
              className="relative flex items-end gap-3 bg-[#141414] border border-white/[0.09] rounded-2xl px-4 py-3 transition-all duration-300"
              style={{}}
              onFocus={() => {}}
              onBlur={() => {}}
            >
              ...
              <div className="chat-input-wrap relative flex items-end gap-3 w-full bg-[#141414] border border-white/[0.09] rounded-2xl px-4 py-3 transition-all duration-300">
                ...
                <button
                  onClick={sendMessage}
                  disabled={
                    !selectedRepo || !input.trim() || loading || processing
                  }
            ...
            <p
              className="text-[10px] text-zinc-700 mt-1.5 ml-1"
              style={{ fontFamily: "'DM Mono', monospace" }}
            >
